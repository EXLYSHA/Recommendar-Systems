...
class SMORE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # modality flags for safer control flow
        self.has_image = self.v_feat is not None
        ...
        # build fusion adjacency depending on available modalities
    ...
    def max_pool_fusion(self):
        """Construct fusion adjacency, robust to missing modalities.
        - If both present: max-pool over edges.
        - If only one present: fall back to that modality's adjacency.
        """
        if self.has_image and self.has_text:
            ...
            return self.image_original_adj
        elif self.has_text:
            return self.text_original_adj
        else:
            raise ValueError('No modality available to build fusion adjacency.')
    ...
    def spectrum_convolution(self, image_embeds, text_embeds):
        ...
        """
        image_conv, text_conv, fusion_conv = None, None, None
        ...
        # Cross-modality fusion: if only one modality exists, fallback to its denoised conv
        if self.has_image and self.has_text:
            fusion_conv = torch.fft.irfft(text_fft * image_fft * fusion_complex_weight,
                                          n=text_embeds.shape[1], dim=1, norm='ortho')
        elif self.has_image:
            fusion_conv = image_conv
        elif self.has_text:
            fusion_conv = text_conv
        else:
            raise ValueError('Both modalities are missing in spectrum_convolution.')
    ...
    def forward(self, adj, train=False):
        ...
        if self.has_image:
            ...
            image_prefer = self.dropout(image_prefer)
            agg_image_embeds = torch.multiply(image_prefer, agg_image_embeds)
            agg_list.append(agg_image_embeds)
        ...
        if self.has_text:
            ...
            text_prefer = self.dropout(text_prefer)
            agg_text_embeds = torch.multiply(text_prefer, agg_text_embeds)
            agg_list.append(agg_text_embeds)
        ...
        agg_list.append(fusion_embeds)

        side_embeds = torch.mean(torch.stack(agg_list), dim=0)
